...
const collectedItems = [
  ...
  {
    item: "텔레그램·Slack 연동 정보 (봇 토큰·웹훅 URL·대화 식별값 등 사용자가 입력한 값)",
    purpose: "원격 발행 조작과 발행 결과 알림 전송",
    storage: "사용자 PC의 앱 설정에 보관됩니다",
  },
  {
    item: "Buffer API 키 (사용자가 입력한 경우)",
    purpose: "카드뉴스 발행 전송 인증",
    storage: "사용자 PC의 앱 설정에 보관됩니다",
  },
  {
    item: "Bitly 토큰 (사용자가 입력한 경우)",
    purpose: "발행된 글 주소의 단축 인증",
    storage: "사용자 PC의 앱 설정에 보관됩니다",
  },
  {
    item: "카드뉴스 이미지 (사용자가 만든 카드)",
    purpose: "SNS 발행을 위해 발행 전송 경로에 전달할 이미지 준비",
    storage:
      "발행 시 사용자 본인의 구글 드라이브에 임시 파일로 업로드되며, 발행 결과가 확정되면 삭제됩니다. 결과 확인이 불가능한 경우에는 남을 수 있습니다",
  },
  {
...
const thirdParties = [
  ...
  },
  {
    name: "Bitly",
    purpose:
      "사용자가 토큰을 입력한 경우 발행된 글 주소의 단축 (단축 실패 시 원문 주소 사용)",
  },
  {
    name: "발행 전송 서비스와 SNS 채널 (예: Buffer를 통한 스레드 발행)",
    purpose: "사용자 요청에 따른 카드뉴스 이미지와 글의 발행 전달",
  },
];
      ...
      <article className="policy" aria-labelledby="privacy-title">
        ...
        <Reveal>
          <section aria-labelledby="privacy-google">
            ...
            <p>
              앱은 두 가지 목적으로 Google OAuth를 사용합니다. 첫째, 사용자가
              지정한 구글 스프레드시트에 글감을 읽고 쓰기 위한 스프레드시트
              접근입니다. 둘째, 카드뉴스 발행 시 완성된 카드 이미지를 사용자
              본인의 구글 드라이브에 임시 파일로 올리기 위한 파일 접근입니다.
              요청 범위는 이 두 목적에 필요한 최소 범위이며, 접근은 항상 사용자
              본인 계정의 권한으로만 수행됩니다. 요청하는 세부 범위는
              스프레드시트 접근과 앱이 만든 파일 접근(drive.file)입니다. 저장된
              토큰은 사용자 PC에만 보관됩니다.
            </p>
            <p>
              카드뉴스 이미지는 발행 전송을 위해 일시적으로 익명 읽기 링크가
              부여된 공개 상태가 되며, 발행 결과가 확정되면 삭제됩니다. 발행
              결과를 확인할 수 없는 경우에는 파일이 남을 수 있습니다.
            </p>
            <p>
